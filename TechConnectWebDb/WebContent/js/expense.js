...
function deleteRow(id,itemClass) {
	...
	if (itemClass == "income") {
	var tbl = document.getElementById('inc-table');
...
var itemId = '';
function createCell(cell, i, title,id,rowCount) {
...
function addSelectAccountBox(cell){
	var select = document.createElement("select");
	var selId = id +'-acc';
	select.setAttribute("name", selId);
	select.setAttribute("id", selId);
	...
	$.ajax({
		  ...
		  data: {flag:"accounts"}
			...
			var jsonTxt = $(msg).text(); 
			var obj = jQuery.parseJSON(jsonTxt);
			$.each(obj.accounts, function(key, value) { 
				var option;
				/* we are going to add two options */
				/* create options elements */
				option = document.createElement("option");
